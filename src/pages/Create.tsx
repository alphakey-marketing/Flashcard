          ...
          {cards.map((card, index) => (
            <div key={card.id} style={styles.cardEditor}>
              ...
              <div style={styles.autoFillContainer}>
                <label style={styles.autoFillLabel}>Japanese vocab (auto-fill)</label>
                <div style={styles.autoFillRow}>
                  <input
                    ...
                    onKeyDown={(e) => { if (e.key === 'Enter') handleAutoFill(card.id); }}
                  />
                  <button
                    type="button"
                ...
                {generateError && <p style={styles.autoFillError}>{generateError}</p>}
              </div>
              <textarea
...
const jlptBadgeColor = (level: string | undefined): CSSProperties => {
  switch (level) {
    case 'N5': return { backgroundColor: '#dcfce7', color: '#166534' };
    case 'N4': return { backgroundColor: '#d1fae5', color: '#065f46' };
    case 'N3': return { backgroundColor: '#dbeafe', color: '#1e40af' };
    case 'N2': return { backgroundColor: '#ede9fe', color: '#5b21b6' };
    case 'N1': return { backgroundColor: '#fee2e2', color: '#991b1b' };
...
const styles: { [key: string]: CSSProperties } = {
  ...
  closeButton: { background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer', color: '#64748b', padding: '4px 8px', transition: 'opacity 0.2s' },
  title: { fontSize: '20px', fontWeight: 600, color: '#0f172a', margin: 0 },
  saveButton: { backgroundColor: '#3b82f6', color: 'white', border: 'none', borderRadius: '8px', padding: '8px 24px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', transition: 'opacity 0.2s' },
  content: { maxWidth: '800px', margin: '0 auto', padding: '24px' },
  section: { marginBottom: '32px' },
  titleInput: { width: '100%', fontSize: '24px', fontWeight: 600, padding: '16px', border: '2px solid #e2e8f0', borderRadius: '12px', marginBottom: '16px', outline: 'none', transition: 'border-color 0.2s', backgroundColor: '#fff', boxSizing: 'border-box' },
  descriptionInput: { width: '100%', fontSize: '16px', padding: '12px 16px', border: '2px solid #e2e8f0', borderRadius: '12px', outline: 'none', resize: 'vertical', fontFamily: 'inherit', transition: 'border-color 0.2s', backgroundColor: '#fff', boxSizing: 'border-box', marginBottom: '16px' },
  levelSelectorContainer: { display: 'flex', flexDirection: 'column', gap: '8px' },
  levelLabel: { fontSize: '14px', fontWeight: 600, color: '#475569' },
  levelSelect: { width: '100%', padding: '12px 16px', fontSize: '16px', border: '2px solid #e2e8f0', borderRadius: '12px', backgroundColor: '#fff', color: '#0f172a', cursor: 'pointer', outline: 'none', transition: 'border-color 0.2s', fontFamily: 'inherit' },
  sectionTitle: { fontSize: '18px', fontWeight: 600, color: '#0f172a', marginBottom: '16px' },
  cardEditor: { backgroundColor: '#fff', borderRadius: '12px', padding: '20px', marginBottom: '16px', border: '1px solid #e2e8f0' },
  cardEditorHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' },
  cardNumber: { fontSize: '14px', fontWeight: 600, color: '#64748b' },
  deleteCardButton: { background: 'none', border: 'none', cursor: 'pointer', fontSize: '16px', padding: '4px', opacity: 0.6, transition: 'opacity 0.2s' },
  cardInput: { width: '100%', fontSize: '14px', padding: '12px', border: '1px solid #e2e8f0', borderRadius: '8px', marginBottom: '8px', outline: 'none', resize: 'vertical', fontFamily: 'inherit', transition: 'border-color 0.2s', boxSizing: 'border-box' },
  addCardButton: { width: '100%', padding: '16px', border: '2px dashed #cbd5e1', borderRadius: '12px', backgroundColor: 'transparent', color: '#3b82f6', fontSize: '16px', fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' },
  autoFillContainer: { marginBottom: '12px' },
  autoFillLabel: { display: 'block', marginBottom: '4px', fontSize: '13px', fontWeight: 600, color: '#64748b' },
  autoFillRow: { display: 'flex', gap: '8px' },
  vocabInput: { flex: 1, padding: '8px 12px', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '15px', fontFamily: 'inherit', outline: 'none' },
  autoFillButton: { padding: '8px 16px', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 600, whiteSpace: 'nowrap', transition: 'background 0.2s' },
  autoFillError: { color: '#dc2626', fontSize: '13px', marginTop: '4px', marginBottom: 0 },
  extractPanel: { marginBottom: '24px', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden', backgroundColor: '#fff' },
  extractToggle: { width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', background: 'none', border: 'none', cursor: 'pointer', fontSize: '15px', fontWeight: 600, color: '#0f172a', fontFamily: 'inherit' },
  extractToggleIcon: { fontSize: '12px', color: '#94a3b8' },
  extractBody: { padding: '0 20px 20px' },
  extractTextarea: { width: '100%', fontSize: '14px', padding: '12px', border: '1px solid #e2e8f0', borderRadius: '8px', outline: 'none', resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: '10px' },
  extractControls: { display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '12px' },
  extractButton: { padding: '8px 20px', backgroundColor: '#4f46e5', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', transition: 'opacity 0.2s' },
  extractToggleLabel: { fontSize: '13px', color: '#64748b', display: 'flex', alignItems: 'center', cursor: 'pointer' },
  extractFallbackNotice: { fontSize: '12px', color: '#92400e', backgroundColor: '#fffbeb', border: '1px solid #fde68a', borderRadius: '6px', padding: '6px 10px', marginBottom: '10px' },
  extractError: { color: '#dc2626', fontSize: '13px', marginBottom: '8px' },
  extractSummary: { fontSize: '13px', color: '#64748b', marginBottom: '10px' },
  extractAiBadge: { display: 'inline-block', marginLeft: '6px', fontSize: '11px', fontWeight: 700, color: '#7c3aed', backgroundColor: '#ede9fe', borderRadius: '4px', padding: '1px 5px' },
  extractList: { maxHeight: '340px', overflowY: 'auto', border: '1px solid #e2e8f0', borderRadius: '8px', marginBottom: '12px' },
  extractRow: { display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '10px 12px', borderBottom: '1px solid #f1f5f9', transition: 'background 0.15s' },
  extractRowSelected: { backgroundColor: '#f0f9ff' },
  extractCheckbox: { marginTop: '3px', flexShrink: 0, cursor: 'pointer' },
  extractRowMain: { flex: 1, minWidth: 0 },
  extractWord: { fontSize: '15px', fontWeight: 700, color: '#0f172a', display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '2px' },
  extractCommonBadge: { fontSize: '10px', fontWeight: 600, color: '#94a3b8', backgroundColor: '#f1f5f9', borderRadius: '4px', padding: '1px 5px' },
  jlptBadge: { fontSize: '10px', fontWeight: 700, borderRadius: '4px', padding: '1px 5px' },
  extractMeta: { display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '2px' },
  extractReading: { fontSize: '12px', color: '#3b82f6', fontWeight: 500 },
  extractMeaning: { fontSize: '12px', color: '#475569' },
  extractExample: { fontSize: '12px', color: '#94a3b8', fontStyle: 'italic', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  extractGenerateBtn: { flexShrink: 0, padding: '5px 10px', border: '1px solid #e2e8f0', borderRadius: '6px', backgroundColor: '#f8fafc', color: '#475569', fontSize: '14px', cursor: 'pointer', transition: 'all 0.2s' },
  extractGenerateBtnDone: { borderColor: '#10b981', color: '#10b981', backgroundColor: '#f0fdf4' },
  extractAddButton: { width: '100%', padding: '11px', backgroundColor: '#3b82f6', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', transition: 'opacity 0.2s', marginBottom: '8px' },
  extractAddSecondary: { width: '100%', padding: '8px', backgroundColor: 'transparent', color: '#64748b', border: '1px solid #e2e8f0', borderRadius: '8px', fontSize: '13px', fontWeight: 500, cursor: 'pointer', transition: 'opacity 0.2s' },
};
